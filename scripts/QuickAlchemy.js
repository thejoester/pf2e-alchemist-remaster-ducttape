...
const acidVialId = "Compendium.pf2e-alchemist-remaster-ducttape.alchemist-duct-tape-fork-items.Item.9NXufURxsBROfbz1";
const poisonVialId = "Compendium.pf2e-alchemist-remaster-ducttape.alchemist-duct-tape-fork-items.Item.LqZyfGxtRGXEpzZq";
...
Hooks.on('renderChatMessage', (message, html) => {
	...
	if (message.isRoll && message.isCheckRoll && message.flags.pf2e?.context?.domains?.includes("healing-bomb-attack")) {
		...
		$html.find(".success[data-outcome=success]").on("click", async (event) => {
	...
	<section class="damage-application" data-roll-index="1">
		...
		$html.find(".failure[data-outcome=failure]").on("click", async (event) => {
			ChatMessage.create({
				content: `
			<section class="dice-roll damage-roll" data-tooltip-class="pf2e">
				<div class="dice-result">	
		...
		$html.find(".success[data-outcome=chirurgeon-success]").on("click", async (event) => {
...
async function craftHealingVial(selectedItem, selectedActor) {
	...
	if (existingItem) {
		...
		const compendium = game.packs.get("pf2e-alchemist-remaster-ducttape.alchemist-duct-tape-fork-items");
		if (!compendium) {
		...
		*/
		...
		if (hasFeat(actor, "advanced-vials-mutagenist")) {
			...
			async function applyEffectFromCompendium(actor) {
				...
				const effectUUID = "Compendium.pf2e-alchemist-remaster-ducttape.alchemist-duct-tape-fork-items.Item.0Zpa9OfNcTUlNc2t";
